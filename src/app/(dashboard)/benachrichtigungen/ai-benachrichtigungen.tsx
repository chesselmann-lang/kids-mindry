...
export default function AiBenachrichtigungen() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/benachrichtigungen-analyse')
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)' }}>
      <div className="bg-white rounded-[14px] p-4">
        ...
        {result && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 text-right pt-1">KI-Analyse · Ihre Benachrichtigungen</p>
          </div>
        )}
      </div>
    </div>
