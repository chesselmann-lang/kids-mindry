...
type Hinweis = { typ: 'kritisch' | 'wichtig' | 'info'; text: string }
type Result = { hinweise: Hinweis[]; stats: { total: number; withAllergies: number; withDiet: number; withNotes: number } }
...
export default function AiAllergienAnalyse() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/allergien-analyse', { method: 'POST' })
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-0.5 shadow-md">
      <div className="rounded-[14px] bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-gray-800">KI-Allergieanalyse</span>
          </div>
          {result && (
            <div className="flex items-center gap-2">
              {result.stats.withAllergies > 0 && (
                <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-amber-100 text-amber-700">
                  {result.stats.withAllergies} betroffen
                </span>
              )}
              {result.stats.withNotes > 0 && (
                <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-red-100 text-red-700">
                  {result.stats.withNotes} med. Notizen
                </span>
              )}
            </div>
          )}
        ...
        {!result && !loading && (
          <button
            onClick={analyse}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Sparkles size={15} />
        ...
        {result && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <button onClick={analyse} className="text-[10px] text-amber-400 hover:text-amber-600 flex items-center gap-1">
