...
const TYP_CONFIG: Record<string, { color: string; dot: string }> = {
  highlight: { color: 'text-pink-800 bg-pink-50',   dot: 'bg-pink-400' },
  info:      { color: 'text-blue-800 bg-blue-50',   dot: 'bg-blue-400' },
  tipp:      { color: 'text-amber-800 bg-amber-50', dot: 'bg-amber-400' },
}

export default function AiGalerie({ albumId }: { albumId: string }) {
  ...
  const analyse = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/ai/galerie-analyse?albumId=${albumId}`)
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #ec4899, #f97316)' }}>
      <div className="bg-white rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #ec4899, #f97316)' }}>
