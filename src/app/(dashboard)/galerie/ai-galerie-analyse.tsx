...
import { Images, RefreshCw, Loader2 } from 'lucide-react'

const TYP_CONFIG: Record<string, { dot: string; bg: string; border: string; text: string }> = {
  tipp:    { dot: 'bg-violet-500', bg: 'bg-violet-50', border: 'border-violet-200', text: 'text-violet-800' },
...
export default function AiGalerieAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/galerie-analyse', { method: 'POST' })
      setData(await res.json())
    } finally { setLoading(false) }
  }

  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg,#8b5cf6,#ec4899)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI Galerie-Analyse</span>
          </div>
          {data?.stats && (
            <div className="flex gap-1.5">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700">
                {data.stats.total} Medien
              </span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-pink-100 text-pink-700">
                {data.stats.recent} neu
              </span>
            </div>
          )}
        </div>
        ...
        <p className="text-[10px] text-gray-400 mt-2">KI-Analyse · Medienübersicht</p>
      </div>
    </div>
  )
}
