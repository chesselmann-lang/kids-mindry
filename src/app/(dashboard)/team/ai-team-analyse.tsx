...
export default function AiTeamAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/team-analyse', { method: 'POST' })
      setData(await res.json())
    } finally { setLoading(false) }
  }

  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg,#0ea5e9,#10b981)' }}>
      <div className="bg-white rounded-[14px] p-4">
        ...
        <p className="text-[10px] text-gray-400 mt-2">KI-Analyse · Teamstruktur</p>
      </div>
    </div>
  )
}
