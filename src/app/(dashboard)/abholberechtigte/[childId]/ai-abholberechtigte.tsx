...
const TYP_CONFIG: Record<string, { color: string; dot: string }> = {
  status:  { color: 'text-green-800 bg-green-50',  dot: 'bg-green-400' },
  hinweis: { color: 'text-amber-800 bg-amber-50',  dot: 'bg-amber-400' },
  info:    { color: 'text-gray-700 bg-gray-50',    dot: 'bg-gray-400' },
}

export default function AiAbholberechtigte({ childId }: { childId: string }) {
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #10b981, #0ea5e9)' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            {data?.stats && (
              <span className="text-[10px] bg-emerald-100 text-emerald-700 font-semibold px-2 py-0.5 rounded-full">
                {data.stats.totalCount} berechtigt
              </span>
            )}
          </div>
          {!data && (
