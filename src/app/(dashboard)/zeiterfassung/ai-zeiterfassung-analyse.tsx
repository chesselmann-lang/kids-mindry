...
export default function AiZeiterfassungAnalyse() {
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Arbeitszeitanalyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Letzte 14 Tage auswerten & Tipps erhalten</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert deine Arbeitszeiten…</span>
      </div>
    )
  }
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-teal-50 to-cyan-50 border-b border-teal-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Sparkles size={14} className="text-teal-500" />
          <span className="text-xs font-semibold text-teal-700">KI-Arbeitszeitanalyse · letzte 14 Tage</span>
        </div>
        <div className="flex items-center gap-2">
          {stats && (
            <span className="text-[10px] text-teal-600 font-medium">
              {stats.entries} Eintr. · Ø {stats.avgHours}h/Tag
            </span>
          )}
      ...
      <div className="p-4 space-y-2.5">
        ...
        <p className="text-[10px] text-gray-400 pt-1">KI-generiert auf Basis deiner Zeiteinträge · Bitte eigenständig prüfen</p>
      </div>
    </div>
  )
}
