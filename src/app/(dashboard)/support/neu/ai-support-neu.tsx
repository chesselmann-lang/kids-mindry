...
interface Hinweis { typ: 'beschreibung' | 'screenshot' | 'schritte' | 'tipp'; text: string }
interface Result { hinweise: Hinweis[] }

const TYP_CONFIG = {
  beschreibung: { dot: 'bg-blue-500',   bg: 'bg-blue-50',   text: 'text-blue-800'   },
  screenshot:   { dot: 'bg-violet-500', bg: 'bg-violet-50', text: 'text-violet-800' },
  schritte:     { dot: 'bg-teal-500',   bg: 'bg-teal-50',   text: 'text-teal-800'   },
  tipp:         { dot: 'bg-gray-400',   bg: 'bg-gray-50',   text: 'text-gray-700'   },
}

export default function AiSupportNeu() {
  ...
  async function laden() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/support-hilfe')
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #6366f1 0%, #0ea5e9 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Hilfe · Support-Anfrage</p>
            <p className="text-[10px] text-gray-400">So beschreiben Sie Ihr Problem am besten</p>
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            {!result && !loading && (
              <button onClick={laden}
                ...
                style={{ background: 'linear-gradient(135deg, #6366f1 0%, #0ea5e9 100%)' }}>
                Tipps für schnelle Hilfe anzeigen
              </button>
            )}
            {loading && (
            ...
            {result && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-300 pt-1">KI-Tipps · Supportanfragen effizient formulieren</p>
              </div>
            )}
          </div>
        )}
