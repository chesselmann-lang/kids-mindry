...
export default function AiSupport() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/support-analyse')
      ...
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI-Support-Überblick</span>
          </div>
          {result && result.stats.open > 0 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 font-semibold px-2 py-0.5 rounded-full">
              {result.stats.open} offen
            </span>
          )}
        </div>
        ...
        {result && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 text-right pt-1">KI-Analyse · Support-Anfragen</p>
          </div>
        )}
      </div>
    </div>
