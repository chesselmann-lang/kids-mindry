...
type Hinweis = { typ: 'tipp' | 'info' | 'vollständig'; text: string }
type Result = { hinweise: Hinweis[]; stats: { total: number; categories: number } }
...
export default function AiFormulareCheck() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/formulare-check', { method: 'POST' })
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-orange-500 to-red-500 p-0.5 shadow-md">
      <div className="rounded-[14px] bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-gray-800">KI-Formular-Check</span>
          </div>
          {result && (
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">
                {result.stats.total} Formulare
              </span>
        ...
        {!result && !loading && (
          <button
            onClick={analyse}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Sparkles size={15} />
            Formulare prüfen
          </button>
