...
const TYP_CONFIG: Record<string, { color: string; dot: string }> = {
  highlight: { color: 'text-amber-800 bg-amber-50',  dot: 'bg-amber-400' },
  tipp:      { color: 'text-blue-800 bg-blue-50',    dot: 'bg-blue-400' },
  info:      { color: 'text-gray-700 bg-gray-50',    dot: 'bg-gray-400' },
}

export default function AiWochenbericht({ reportId }: { reportId: string }) {
  ...
  const analyse = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/ai/wochenbericht-analyse?reportId=${reportId}`)
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #f59e0b, #8b5cf6)' }}>
      <div className="bg-white rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f59e0b, #8b5cf6)' }}>
            ...
            {data?.stats && (
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[10px] bg-amber-100 text-amber-700 font-semibold px-2 py-0.5 rounded-full">
                  {data.stats.highlightCount} Highlights
                </span>
                {data.stats.hasPhotos && (
