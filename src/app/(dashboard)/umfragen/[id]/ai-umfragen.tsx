...
const TYP_CONFIG: Record<string, { color: string; dot: string }> = {
  ergebnis: { color: 'text-emerald-800 bg-emerald-50', dot: 'bg-emerald-400' },
  tipp:     { color: 'text-blue-800 bg-blue-50',      dot: 'bg-blue-400' },
  info:     { color: 'text-gray-700 bg-gray-50',      dot: 'bg-gray-400' },
}

export default function AiUmfragen({ pollId }: { pollId: string }) {
  ...
  const analyse = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/ai/umfragen-analyse?pollId=${pollId}`)
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #10b981, #6366f1)' }}>
      <div className="bg-white rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #10b981, #6366f1)' }}>
            ...
            {data?.stats && (
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[10px] bg-emerald-100 text-emerald-700 font-semibold px-2 py-0.5 rounded-full">
                  {data.stats.totalVoters} Stimmen
                </span>
                {data.stats.isClosed && (
