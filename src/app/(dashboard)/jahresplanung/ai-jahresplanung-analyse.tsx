...
export default function AiJahresplanungAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/jahresplanung-analyse', { method: 'POST' })
      if (!res.ok) return
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg,#8b5cf6,#6366f1)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI Jahresplan-Check</span>
          </div>
          {data?.stats && (
            <div className="flex gap-1.5">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700">
                {data.stats.total} Einträge
              </span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">
                {data.stats.months}/12 Monate
              </span>
            </div>
          )}
        </div>
        ...
        <p className="text-[10px] text-gray-400 mt-2">KI-Analyse · Jahresplanung</p>
      </div>
    </div>
  )
}
