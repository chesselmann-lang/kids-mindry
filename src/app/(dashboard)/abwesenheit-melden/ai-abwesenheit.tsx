...
import { Sparkles, UserX, Loader2 } from 'lucide-react'

const TYP_CONFIG: Record<string, { color: string; dot: string }> = {
  muster: { color: 'text-amber-800 bg-amber-50',  dot: 'bg-amber-400' },
  tipp:   { color: 'text-blue-800 bg-blue-50',    dot: 'bg-blue-400' },
  info:   { color: 'text-gray-700 bg-gray-50',    dot: 'bg-gray-400' },
}

export default function AiAbwesenheit() {
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)' }}>
      <div className="bg-white rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)' }}>
              <UserX size={14} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-gray-800">KI-Analyse (30 Tage)</span>
            {data?.stats && (
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] bg-orange-100 text-orange-700 font-semibold px-2 py-0.5 rounded-full">
                  {data.stats.absenceCount} Fehlzeiten
                </span>
              </div>
            )}
          </div>
