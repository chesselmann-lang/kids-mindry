...
import { Euro, Loader2, RefreshCw } from 'lucide-react'
...
interface Result {
  hinweise: Hinweis[]
  stats: { totalOpen: number; totalPaid: number; overdue: number; paymentRate: number }
  message?: string
}
...
export default function AiGebuehrenAnalyse() {
  ...
  if (!loaded && !loading) {
    return (
      ...
      >
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-emerald-100 to-green-100 flex items-center justify-center flex-shrink-0">
          <Euro size={18} className="text-emerald-600" />
        </div>
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Gebühren-Analyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Rückstände & Zahlungsrate prüfen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Gebühren…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <Euro size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-emerald-50 to-green-50 border-b border-emerald-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-emerald-700">KI-Gebühren-Analyse</span>
          ...
          <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-medium">
            {result.stats?.paymentRate ?? 0}% bezahlt
          </span>
        </div>
