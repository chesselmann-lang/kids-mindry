...
interface Result {
  hinweise: Hinweis[]
  stats: { total: number; expiring: number; expired: number }
  message?: string
}
...
export default function AiDokumenteCheck() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/dokumente-check', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Dokumente-Check</p>
          <p className="text-xs text-gray-400 mt-0.5">Ablaufende & fehlende Dokumente</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI prüft Dokumente…</span>
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-orange-700">KI-Dokumente-Check</span>
          {(result.stats?.expired ?? 0) > 0 && (
            <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.expired} abgelaufen
            </span>
          )}
          {(result.stats?.expiring ?? 0) > 0 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.expiring} läuft ab
            </span>
          )}
        </div>
