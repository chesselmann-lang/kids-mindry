...
export default function AiGuardian({ childId }: { childId: string }) {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch(`/api/ai/guardian-analyse?childId=${childId}`)
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #6366f1, #0ea5e9)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #6366f1, #0ea5e9)' }}>
              <Users size={14} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-gray-800">KI-Vollständigkeitsprüfung</span>
          </div>
          {result && (
            <div className="flex gap-1.5">
              <span className="text-[10px] bg-indigo-100 text-indigo-700 font-semibold px-2 py-0.5 rounded-full">
                {result.stats.total} Erziehungsber.
              </span>
              {result.stats.withoutConsent > 0 && (
                <span className="text-[10px] bg-red-100 text-red-700 font-semibold px-2 py-0.5 rounded-full">
                  {result.stats.withoutConsent} ohne Einwilligung
                </span>
              )}
            </div>
          )}
        ...
        {result && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 text-right pt-1">KI-Analyse · Datenvollständigkeit</p>
          </div>
        )}
      </div>
    </div>
