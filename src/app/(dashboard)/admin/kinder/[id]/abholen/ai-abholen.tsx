...
import { UserCheck, Loader2, ChevronDown, ChevronUp } from 'lucide-react'

interface Hinweis { typ: 'status' | 'hinweis' | 'info'; text: string }
interface Result { hinweise: Hinweis[]; stats: { total: number; withPhone: number; relationships: string[] } }

const TYP_CONFIG = {
  status:  { dot: 'bg-emerald-500', bg: 'bg-emerald-50',  text: 'text-emerald-800' },
  hinweis: { dot: 'bg-amber-500',   bg: 'bg-amber-50',    text: 'text-amber-800'   },
  info:    { dot: 'bg-gray-400',    bg: 'bg-gray-50',     text: 'text-gray-700'    },
}

export default function AiAbholen({ childId }: { childId: string }) {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch(`/api/ai/abholberechtigte-analyse?childId=${childId}`)
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #10b981 0%, #0ea5e9 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Analyse · Abholberechtigte</p>
            {result && <p className="text-[10px] text-gray-400">{result.stats.total} Personen · {result.stats.withPhone} mit Telefon</p>}
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            ...
            {result && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-300 pt-1">KI-Hinweise · keine Rechtsberatung</p>
              </div>
            )}
          </div>
        )}
