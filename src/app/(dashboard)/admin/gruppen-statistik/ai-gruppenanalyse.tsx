...
import { Sparkles, Loader2, RefreshCw, CheckCircle2, Info, Zap } from 'lucide-react'

interface Erkenntnis {
  typ: 'positiv' | 'hinweis' | 'handlung'
  titel: string
  text: string
  gruppe: string | null
}
...
const TYP_CONFIG = {
  ...
  handlung: { icon: Zap,           bg: 'bg-amber-50',  border: 'border-amber-100',  text: 'text-amber-700',  iconColor: 'text-amber-500' },
}

export default function AiGruppenanalyse() {
  const [erkenntnisse, setErkenntnisse] = useState<Erkenntnis[]>([])
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/gruppenanalyse', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      })
      const data = await res.json()
      if (!res.ok || !data.erkenntnisse) throw new Error('failed')
      setErkenntnisse(data.erkenntnisse)
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Gruppenanalyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Datenbasierte Erkenntnisse zur Gruppenentwicklung</p>
        </div>
      </button>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-indigo-50 to-violet-50 border-b border-indigo-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-indigo-700">KI-Gruppenanalyse · {monthLabel}</span>
        </div>
      ...
      <div className="p-4 space-y-2.5">
        {erkenntnisse.length === 0 ? (
          <p className="text-xs text-gray-400">Keine Auffälligkeiten diesen Monat.</p>
        ) : erkenntnisse.map((e, i) => {
          const cfg = TYP_CONFIG[e.typ] ?? TYP_CONFIG.hinweis
          const Icon = cfg.icon
          return (
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className={`text-xs font-semibold ${cfg.text}`}>{e.titel}</p>
                    {e.gruppe && (
                  ...
                  <p className="text-xs text-gray-700 leading-relaxed">{e.text}</p>
                </div>
              </div>
            </div>
          )
        })}
        <p className="text-[10px] text-gray-400 pt-1">KI-generiert auf Basis der Monatsdaten · Bitte eigenständig prüfen</p>
      </div>
    </div>
  )
}
