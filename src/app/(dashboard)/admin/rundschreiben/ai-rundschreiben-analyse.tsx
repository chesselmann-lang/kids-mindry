...
interface Hinweis {
  typ: 'hinweis' | 'positiv' | 'info'
  text: string
}

interface Result {
  hinweise: Hinweis[]
  stats: { total: number; pushCount: number; parentCount: number; pushRate: number }
  message?: string
}
...
export default function AiRundschreibenAnalyse() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/rundschreiben-analyse', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Kommunikations-Check</p>
          <p className="text-xs text-gray-400 mt-0.5">Frequenz & Push-Reichweite prüfen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Kommunikation…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <Send size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
