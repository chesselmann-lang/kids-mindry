...
export default function AiDatenschutzCheck() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/ai/datenschutz-check')
  ...
  return (
    <div className="rounded-2xl p-0.5 mb-1" style={{ background: 'linear-gradient(135deg, #10b981, #0ea5e9)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900">Datenschutz-Check</p>
              <p className="text-[10px] text-gray-400">KI-Analyse der Einwilligungslage</p>
            </div>
          </div>
          {data && (
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">{data.stats.consentRate}% vollständig</span>
              {data.stats.consentMissing > 0 && (
                <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-red-100 text-red-700">{data.stats.consentMissing} fehlen</span>
              )}
            </div>
          )}
        </div>
        ...
        {!data && !loading && (
          <button onClick={load} className="w-full py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: 'linear-gradient(135deg, #10b981, #0ea5e9)' }}>
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 py-4 text-emerald-600">
            ...
            <span className="text-sm">Prüfe Einwilligungsstatus…</span>
          </div>
        )}
