...
import { Target, Loader2, RefreshCw, AlertCircle, Lightbulb, Info } from 'lucide-react'
...
interface Result {
  ...
  hinweise: Hinweis[]
  stats: { total: number; overdue: number; dueSoon: number; expiring: number }
  message?: string
}

const TYP_CONFIG = {
  dringend:    { dot: 'bg-red-400',    bg: 'bg-red-50',    border: 'border-red-100',    text: 'text-red-700' },
  empfehlung:  { dot: 'bg-violet-400', bg: 'bg-violet-50', border: 'border-violet-100', text: 'text-violet-700' },
  info:        { dot: 'bg-blue-400',   bg: 'bg-blue-50',   border: 'border-blue-100',   text: 'text-blue-700' },
}

export default function AiFoerderplaeneUeberblick() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/foerderplaene-ueberblick', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Förderplan-Überblick</p>
          <p className="text-xs text-gray-400 mt-0.5">Stand und Überprüfungsbedarf analysieren</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Förderpläne…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <Target size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-violet-50 to-purple-50 border-b border-violet-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Target size={14} className="text-violet-500" />
          <span className="text-xs font-semibold text-violet-700">KI-Förderplan-Überblick · {result.stats?.total ?? 0} Pläne</span>
          {(result.stats?.overdue ?? 0) > 0 && (
            <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.overdue} überfällig
            </span>
          )}
          {(result.stats?.dueSoon ?? 0) > 0 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.dueSoon} bald fällig
            </span>
          )}
        </div>
        <button onClick={analyse} className="p-1.5 rounded-lg hover:bg-violet-100 transition-colors">
