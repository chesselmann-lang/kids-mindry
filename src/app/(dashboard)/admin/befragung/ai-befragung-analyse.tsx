...
type Result = { hinweise: Hinweis[]; stats: { total: number; active: number; totalResponses: number; avgResponseRate: number } }
...
export default function AiBefragungAnalyse() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/befragung-analyse', { method: 'POST' })
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-600 p-0.5 shadow-md">
      <div className="rounded-[14px] bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-gray-800">KI-Befragungsanalyse</span>
          </div>
          {result && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                {result.stats.totalResponses} Antworten
              </span>
            </div>
          )}
        </div>
        ...
        {!result && !loading && (
          <button
            onClick={analyse}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Sparkles size={15} />
