...
import { RefreshCw, Loader2, AlertTriangle, Info, CheckCircle2 } from 'lucide-react'
...
interface Result {
  hinweise: Hinweis[]
  stats: { total: number; uncovered: number; today: number }
  message?: string
}
...
export default function AiVertretungAnalyse() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/vertretung-analyse', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Vertretungs-Check</p>
          <p className="text-xs text-gray-400 mt-0.5">Lücken & Engpässe erkennen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI prüft Vertretungsplan…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <RefreshCw size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-cyan-50 to-teal-50 border-b border-cyan-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-cyan-700">KI-Vertretungs-Check</span>
          {(result.stats?.uncovered ?? 0) > 0 && (
            <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.uncovered} unbesetzt
            </span>
          )}
      ...
      <div className="p-4 space-y-2.5">
        ...
        <p className="text-[10px] text-gray-400 pt-1">KI-generiert · Nächste 30 Tage</p>
      </div>
    </div>
  )
}
