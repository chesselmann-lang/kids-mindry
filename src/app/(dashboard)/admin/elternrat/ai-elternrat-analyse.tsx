...
import { Users2, Loader2, RefreshCw } from 'lucide-react'
...
interface Result {
  hinweise: Hinweis[]
  stats: { members: number; meetings: number; expiringMembers: number }
  message?: string
}
...
export default function AiElternratAnalyse() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/elternrat-analyse', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Elternrat-Check</p>
          <p className="text-xs text-gray-400 mt-0.5">Mandate & Sitzungsplanung prüfen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Elternrat…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <Users2 size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-teal-50 to-emerald-50 border-b border-teal-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-teal-700">KI-Elternrat-Check</span>
          <span className="text-[10px] bg-teal-100 text-teal-700 px-1.5 py-0.5 rounded-full font-medium">
            {result.stats?.members ?? 0} Mitglieder
          </span>
          {(result.stats?.expiringMembers ?? 0) > 0 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.expiringMembers} Mandat läuft ab
            </span>
          )}
        </div>
