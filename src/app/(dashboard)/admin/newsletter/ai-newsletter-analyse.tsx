...
interface Result {
  hinweise: Hinweis[]
  stats: { total: number; daysSinceLast: number; avgIntervalDays: number }
  message?: string
}
...
export default function AiNewsletterAnalyse() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/newsletter-analyse', {
        method: 'POST',
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Newsletter-Analyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Frequenz & Engagement prüfen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Newsletter…</span>
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-violet-50 to-purple-50 border-b border-violet-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-violet-700">KI-Newsletter-Analyse</span>
          {(result.stats?.daysSinceLast ?? 0) > 30 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.daysSinceLast} Tage seit letztem
            </span>
          )}
          <span className="text-[10px] bg-violet-100 text-violet-700 px-1.5 py-0.5 rounded-full font-medium">
            {result.stats?.total ?? 0} gesamt
          </span>
        </div>
