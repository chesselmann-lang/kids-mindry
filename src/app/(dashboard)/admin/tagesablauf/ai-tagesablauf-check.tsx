...
import { Clock3, Loader2, RefreshCw, CheckCircle2, AlertCircle, Info } from 'lucide-react'

interface Hinweis {
  typ: 'verbesserung' | 'gut' | 'info'
  text: string
}
...
const TYP_CONFIG = {
  verbesserung: { dot: 'bg-amber-400',  bg: 'bg-amber-50',  border: 'border-amber-100',  text: 'text-amber-700' },
...
export default function AiTagesablaufCheck() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/tagesablauf-check', {
        method: 'POST',
  ...
  if (!loaded && !loading) {
    return (
      ...
      >
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-green-100 to-emerald-100 flex items-center justify-center flex-shrink-0">
          <Clock3 size={18} className="text-green-600" />
        </div>
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Tagesablauf-Check</p>
          <p className="text-xs text-gray-400 mt-0.5">Lücken & Ausgewogenheit prüfen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Tagesablauf…</span>
      </div>
    )
  }
  ...
  if (result.message) {
    return (
      <div className="card p-4 text-sm text-gray-500 flex items-center gap-2">
        <Clock3 size={16} className="text-gray-300 flex-shrink-0" />
        {result.message}
      </div>
    )
  }
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-green-700">KI-Tagesablauf-Check</span>
          <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-medium">
            {result.stats?.total ?? 0} Einträge
          </span>
        </div>
        <button onClick={analyse} className="p-1.5 rounded-lg hover:bg-green-100 transition-colors">
          <RefreshCw size={13} className="text-green-500" />
        </button>
      </div>
      ...
      <div className="p-4 space-y-2.5">
        ...
        <p className="text-[10px] text-gray-400 pt-1">KI-generiert · Pädagogische Einschätzung</p>
      </div>
    </div>
  )
}
