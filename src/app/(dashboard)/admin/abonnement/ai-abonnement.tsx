...
export default function AiAbonnement() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/abonnement-analyse')
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #8b5cf6, #3b82f6)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI-Abo-Analyse</span>
          </div>
          {result && (
        ...
        {result && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 text-right pt-1">KI-Analyse · Abonnement-Status</p>
          </div>
        )}
      </div>
    </div>
