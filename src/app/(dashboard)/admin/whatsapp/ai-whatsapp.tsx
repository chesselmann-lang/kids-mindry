...
interface Hinweis { typ: 'formulierung' | 'timing' | 'inhalt' | 'tipp'; text: string }
interface Result { hinweise: Hinweis[] }

const TYP_CONFIG = {
  formulierung: { dot: 'bg-green-500',   bg: 'bg-green-50',   text: 'text-green-800'   },
  timing:       { dot: 'bg-blue-500',    bg: 'bg-blue-50',    text: 'text-blue-800'    },
  inhalt:       { dot: 'bg-purple-500',  bg: 'bg-purple-50',  text: 'text-purple-800'  },
...
export default function AiWhatsapp() {
  ...
  async function laden() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/whatsapp-tipps')
      ...
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Tipps · WhatsApp Broadcast</p>
            <p className="text-[10px] text-gray-400">Effektiv mit Eltern kommunizieren</p>
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            ...
            {result && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-300 pt-1">KI-Kommunikation · Best Practices für Kita-Broadcast</p>
              </div>
            )}
          </div>
        )}
