...
interface Hinweis { typ: 'live' | 'hinweis' | 'info'; text: string }
interface Result { hinweise: Hinweis[]; stats: { presentCount: number; absentCount: number; unknownCount: number; totalCount: number; attendanceRate: number } }

const TYP_CONFIG = {
  live:    { dot: 'bg-green-500',  bg: 'bg-green-50',  text: 'text-green-800'  },
  hinweis: { dot: 'bg-amber-500',  bg: 'bg-amber-50',  text: 'text-amber-800'  },
  info:    { dot: 'bg-gray-400',   bg: 'bg-gray-50',   text: 'text-gray-700'   },
}

export default function AiLiveAnwesenheit() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/live-anwesenheit-analyse')
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #22c55e 0%, #3b82f6 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Analyse · Heutige Anwesenheit</p>
            {result && (
              <p className="text-[10px] text-gray-400">
                {result.stats.presentCount} anwesend · {result.stats.absentCount} abwesend · {result.stats.unknownCount} unbekannt ({result.stats.attendanceRate}%)
              </p>
            )}
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            ...
            {result && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-300 pt-1">KI-Hinweise · Echtzeit-Schnappschuss</p>
              </div>
            )}
          </div>
        )}
