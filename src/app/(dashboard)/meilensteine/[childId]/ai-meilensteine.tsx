...
type Hinweis = { typ: 'beobachtung' | 'empfehlung' | 'info'; text: string }

const TYP_CONFIG = {
  beobachtung: { dot: 'bg-yellow-400', badge: 'bg-yellow-50 text-yellow-800', label: 'Beobachtung' },
  empfehlung:  { dot: 'bg-purple-400', badge: 'bg-purple-50 text-purple-800', label: 'Empfehlung' },
  info:        { dot: 'bg-blue-400',   badge: 'bg-blue-50 text-blue-800',     label: 'Info' },
}

export default function AiMeilensteine({ childId }: { childId: string }) {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch(`/api/ai/meilensteine-analyse?childId=${childId}`)
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #f59e0b, #8b5cf6)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f59e0b, #8b5cf6)' }}>
              <Star size={14} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-gray-800">KI-Meilenstein-Analyse</span>
          </div>
          {result && (
            <div className="flex gap-1.5">
              <span className="text-[10px] bg-yellow-100 text-yellow-700 font-semibold px-2 py-0.5 rounded-full">
                {result.stats.achieved} erreicht
              </span>
              {result.stats.pending > 0 && (
                <span className="text-[10px] bg-purple-100 text-purple-700 font-semibold px-2 py-0.5 rounded-full">
                  {result.stats.pending} ausstehend
                </span>
              )}
            </div>
          )}
