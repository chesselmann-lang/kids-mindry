...
import { Thermometer, RefreshCw, Loader2 } from 'lucide-react'
...
export default function AiKrankmeldungAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/krankmeldung-analyse', { method: 'POST' })
      if (!res.ok) return
      ...
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI Krankmeldungs-Check</span>
          </div>
          {data?.stats && (
        ...
        <p className="text-[10px] text-gray-400 mt-2">KI-Analyse · Abwesenheitsübersicht</p>
      </div>
    </div>
  )
}
