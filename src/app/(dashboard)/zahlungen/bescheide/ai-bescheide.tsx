...
interface Hinweis { typ: 'zeitpunkt' | 'inhalt' | 'recht' | 'tipp'; text: string }
interface Result { hinweise: Hinweis[] }

const TYP_CONFIG = {
  zeitpunkt: { dot: 'bg-blue-500',   bg: 'bg-blue-50',   text: 'text-blue-800'   },
  inhalt:    { dot: 'bg-green-500',  bg: 'bg-green-50',  text: 'text-green-800'  },
  recht:     { dot: 'bg-amber-500',  bg: 'bg-amber-50',  text: 'text-amber-800'  },
  tipp:      { dot: 'bg-gray-400',   bg: 'bg-gray-50',   text: 'text-gray-700'   },
}

export default function AiBescheide() {
  ...
  async function laden() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/bescheide-tipps')
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #0ea5e9 0%, #059669 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Hinweise · Elternbescheide</p>
            <p className="text-[10px] text-gray-400">Zeitpunkt, Inhalt und rechtliche Anforderungen</p>
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            {!result && !loading && (
              <button onClick={laden}
                ...
                style={{ background: 'linear-gradient(135deg, #0ea5e9 0%, #059669 100%)' }}>
                Bescheid-Hinweise anzeigen
              </button>
            )}
            {loading && (
              <div className="flex items-center justify-center gap-2 py-3">
                <Loader2 size={14} className="animate-spin text-sky-500" />
                <span className="text-xs text-gray-500">Lade Hinweise…</span>
              </div>
            )}
            {result && (
