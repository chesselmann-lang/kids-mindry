...
export default function AiSteuer({ year }: { year: number }) {
  ...
  const analyse = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/ai/steuer-analyse?year=${year}`)
  ...
  return (
    <div className="rounded-2xl p-[2px]" style={{ background: 'linear-gradient(135deg, #22c55e, #0ea5e9)' }}>
      <div className="bg-white rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #22c55e, #0ea5e9)' }}>
              <Receipt size={14} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-gray-800">KI-Steuerhinweise</span>
            {data?.stats && (
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] bg-green-100 text-green-700 font-semibold px-2 py-0.5 rounded-full">
                  {data.stats.totalEuro} € ({data.stats.year})
                </span>
              </div>
            )}
          </div>
          {!data && (
            ...
            >
              ...
              {loading ? 'Analysiere…' : 'Steuerhinweise'}
            </button>
          )}
        </div>
        ...
        {data?.hinweise && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 mt-1">KI-generiert · Kein Steuerrecht — bitte Steuerberater fragen</p>
          </div>
        )}
      </div>
    </div>
