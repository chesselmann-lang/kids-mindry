...
import { Euro, Loader2, ChevronDown, ChevronUp } from 'lucide-react'

interface Hinweis { typ: 'benennung' | 'betrag' | 'faelligkeit' | 'tipp'; text: string }
interface Result { hinweise: Hinweis[] }

const TYP_CONFIG = {
  benennung:  { dot: 'bg-blue-500',   bg: 'bg-blue-50',   text: 'text-blue-800'   },
  betrag:     { dot: 'bg-green-500',  bg: 'bg-green-50',  text: 'text-green-800'  },
  faelligkeit:{ dot: 'bg-amber-500',  bg: 'bg-amber-50',  text: 'text-amber-800'  },
  tipp:       { dot: 'bg-gray-400',   bg: 'bg-gray-50',   text: 'text-gray-700'   },
}

export default function AiZahlungNeu() {
  ...
  async function laden() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/zahlung-tipps')
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #10b981 0%, #3b82f6 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Tipps · Zahlungsposten</p>
            <p className="text-[10px] text-gray-400">Zahlungen korrekt und eindeutig erfassen</p>
          </div>
          {result && (
          ...
          <div className="px-4 pb-3">
            ...
            {result && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-gray-300 pt-1">KI-Tipps · keine Buchführungsberatung</p>
              </div>
            )}
          </div>
        )}
