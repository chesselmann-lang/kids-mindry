...
import { Euro, RefreshCw, Loader2 } from 'lucide-react'
...
export default function AiZahlungenAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/zahlungen-analyse', { method: 'POST' })
      if (!res.ok) return
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg,#10b981,#059669)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI Zahlungs-Check</span>
          </div>
          {data?.stats && (
            <div className="flex gap-1.5">
              {data.stats.overdue > 0 && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700">
                  {data.stats.overdue} überfällig
                </span>
              )}
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                {data.stats.open} offen
              </span>
            </div>
          )}
        </div>
        ...
        <p className="text-[10px] text-gray-400 mt-2">KI-Analyse · keine Finanzberatung</p>
      </div>
    </div>
  )
}
