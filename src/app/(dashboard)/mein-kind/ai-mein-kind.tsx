...
type Hinweis = { typ: 'info' | 'tipp' | 'hinweis'; text: string }

const TYP_CONFIG = {
  info:    { dot: 'bg-blue-400',   badge: 'bg-blue-50 text-blue-800',     label: 'Info' },
  tipp:    { dot: 'bg-purple-400', badge: 'bg-purple-50 text-purple-800', label: 'Tipp' },
  hinweis: { dot: 'bg-amber-400',  badge: 'bg-amber-50 text-amber-800',   label: 'Hinweis' },
}

export default function AiMeinKind() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/mein-kind-analyse')
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #f472b6, #8b5cf6)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #f472b6, #8b5cf6)' }}>
              <Baby size={14} className="text-white" />
            </div>
            <span className="text-sm font-semibold text-gray-800">KI-Überblick</span>
          </div>
          {result && result.stats.totalDays > 0 && (
            <div className="flex gap-1.5">
              <span className="text-[10px] bg-green-100 text-green-700 font-semibold px-2 py-0.5 rounded-full">
                {result.stats.presentDays} Tage anwesend
              </span>
            </div>
          )}
        </div>
        ...
        {result && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-gray-400 text-right pt-1">KI-Analyse · Letzte 30 Tage</p>
          </div>
        )}
      </div>
    </div>
