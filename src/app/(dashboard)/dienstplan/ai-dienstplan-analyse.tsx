...
export default function AiDienstplanAnalyse({ weekStart, isAdmin }: Props) {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/dienstplan-analyse', {
        method: 'POST',
  ...
  if (!loaded && !loading) {
    return (
      ...
      >
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-100 to-violet-100 flex items-center justify-center flex-shrink-0">
          <Sparkles size={18} className="text-purple-600" />
        </div>
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Dienstplan Analyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Abdeckung, Fachkraft-Ratio und Optimierungshinweise</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Dienstplan…</span>
      </div>
    )
  }
  ...
  if (message && hinweise.length === 0) {
    return (
      <div className="card p-4 text-sm text-gray-500 bg-gray-50 flex items-center justify-between">
        <span>{message}</span>
        <button onClick={analyse} className="p-1 hover:text-purple-600">
          <RefreshCw size={13} />
        </button>
      </div>
    )
    ...
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-purple-50 to-violet-50 border-b border-purple-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-purple-700">KI-Dienstplan Analyse · {weekLabel}</span>
        </div>
