...
type Hinweis = { typ: 'hinweis' | 'tipp' | 'info'; text: string }
...
export default function AiIntern({ groupId }: { groupId: string }) {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch(`/api/ai/intern-analyse?groupId=${groupId}`)
  ...
  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
      <div className="bg-white rounded-[14px] p-4">
        ...
        {!result && !loading && (
          <button onClick={analyse}
            ...
            <Sparkles size={14} />
            Team-Kommunikation analysieren
          </button>
        )}
