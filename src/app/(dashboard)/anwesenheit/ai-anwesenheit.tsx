...
interface Hinweis { typ: 'muster' | 'tipp' | 'info'; text: string }
interface Result { hinweise: Hinweis[]; stats: { absenceCount: number; sickCount: number; vacationCount: number } }

const TYP_CONFIG = {
  muster: { dot: 'bg-orange-500', bg: 'bg-orange-50',  text: 'text-orange-800' },
  tipp:   { dot: 'bg-blue-500',   bg: 'bg-blue-50',    text: 'text-blue-800'   },
  info:   { dot: 'bg-gray-400',   bg: 'bg-gray-50',    text: 'text-gray-700'   },
}

export default function AiAnwesenheit() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/abwesenheit-analyse')
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #f97316 0%, #ef4444 100%)' }}>
      <div className="m-0.5 rounded-[14px] bg-white">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-gray-800">KI-Analyse · Fehlzeiten (30 Tage)</p>
            {result && (
              <p className="text-[10px] text-gray-400">
                {result.stats.absenceCount} Fehlzeiten · {result.stats.sickCount} Krank · {result.stats.vacationCount} Urlaub
              </p>
            )}
          </div>
          {result && (
