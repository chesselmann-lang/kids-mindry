...
export default function AiProtokolleAnalyse() {
  ...
  async function analyse() {
    ...
    setError(false)
    try {
      const res = await fetch('/api/ai/protokolle-analyse', {
        method: 'POST',
  ...
  if (!loaded && !loading) {
    return (
      ...
      >
        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-slate-100 to-gray-100 flex items-center justify-center flex-shrink-0">
          <FileText size={18} className="text-slate-600" />
        </div>
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-gray-900">KI-Protokoll-Analyse</p>
          <p className="text-xs text-gray-400 mt-0.5">Offene Punkte & Entwürfe erkennen</p>
        </div>
      </button>
    )
  }
  ...
  if (loading) {
    return (
      <div className="card p-4 flex items-center gap-3 text-gray-400">
        ...
        <span className="text-sm">KI analysiert Protokolle…</span>
      </div>
    )
  }
  ...
  return (
    <div className="card overflow-hidden">
      <div className="px-4 py-3 bg-gradient-to-r from-slate-50 to-gray-50 border-b border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <FileText size={14} className="text-slate-500" />
          <span className="text-xs font-semibold text-slate-700">KI-Protokoll-Analyse</span>
          {(result.stats?.drafts ?? 0) > 0 && (
            <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-medium">
              {result.stats.drafts} Entwurf
            </span>
          )}
          <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full font-medium">
            {result.stats?.published ?? 0} veröff.
          </span>
        </div>
        <button onClick={analyse} className="p-1.5 rounded-lg hover:bg-slate-100 transition-colors">
