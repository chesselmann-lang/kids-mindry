...
export default function AiRegelwerkAnalyse() {
  ...
  async function analyse() {
    setLoading(true)
    try {
      const res = await fetch('/api/ai/regelwerk-analyse', { method: 'POST' })
      setData(await res.json())
    } finally { setLoading(false) }
  }

  return (
    <div className="rounded-2xl p-0.5" style={{ background: 'linear-gradient(135deg,#64748b,#475569)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-800">KI Regelwerk-Check</span>
          </div>
          {data?.stats && (
            <div className="flex gap-1.5">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">
                {data.stats.total} Regeln
              </span>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                {data.stats.categories} Kategorien
              </span>
            </div>
          )}
        </div>
