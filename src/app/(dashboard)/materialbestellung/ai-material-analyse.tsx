...
type Result = { hinweise: Hinweis[]; stats: { pending: number; ordered: number; highPriority: number; overdue: number } }

const TYP_CONFIG = {
  dringend: { dot: 'bg-red-400',   bg: 'bg-red-50',   border: 'border-red-200',   text: 'text-red-800' },
...
export default function AiMaterialAnalyse() {
  ...
  async function analyse() {
    ...
    try {
      const res = await fetch('/api/ai/material-analyse', { method: 'POST' })
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 p-0.5 shadow-md">
      <div className="rounded-[14px] bg-white p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-gray-800">KI-Bestellanalyse</span>
          </div>
          {result && (
            <div className="flex items-center gap-2">
        ...
        {!result && !loading && (
          <button
            onClick={analyse}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Sparkles size={15} />
            Bestellungen prüfen
          </button>
        ...
        {result && (
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between pt-1">
              ...
              <button onClick={analyse} className="text-[10px] text-emerald-400 hover:text-emerald-600 flex items-center gap-1">
