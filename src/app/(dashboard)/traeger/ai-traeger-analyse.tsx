...
export default function AiTraegerAnalyse() {
  ...
  async function load() {
    ...
    setError(null)
    try {
      const res = await fetch('/api/ai/traeger-analyse')
  ...
  return (
    <div className="rounded-2xl p-0.5 mb-1" style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)' }}>
      <div className="bg-white rounded-[14px] p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-bold text-gray-900">Träger-Analyse</p>
              <p className="text-[10px] text-gray-400">KI-Übersicht über alle Kitas</p>
            </div>
          </div>
          {data && (
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-purple-100 text-purple-700">{data.stats.totalKitas} Kitas</span>
              <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-indigo-100 text-indigo-700">{data.stats.totalKinder} Kinder</span>
            </div>
          )}
        </div>

        {!data && !loading && (
          <button onClick={load} className="w-full py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: 'linear-gradient(135deg, #7c3aed, #4f46e5)' }}>
            Träger-Übersicht analysieren
          </button>
        )}
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 py-4 text-purple-600">
            ...
            <span className="text-sm">Analysiere alle Kitas…</span>
          </div>
        )}
